...
// Mudar o fundo da navbar conforme o usuário rola a página
...
// Exibir imóveis em destaque na página principal
function gerarDestaques(titulo, ids) {
  const imoveisFiltrados = imoveis.filter(imovel => ids.includes(imovel.id));
  const destaquesHTML = imoveisFiltrados.map(imovel => `
      <div class="col-md-4">
          <div class="card h-100 rounded-0 border-gray">
              ...
              <div class="card-body px-5 py-4">
                  ...
                  <p class="card-text small my-2">${imovel.area}m² • ${imovel.quartos} Quartos • ${imovel.banheiros} Banheiros • ${imovel.vagas} Vaga(s)</p>
                  <p class="card-text small fw-semibold m-0">${imovel.pretensao}</p>
                  <h5 class="card-title">${imovel.pretensao === 'Aluguel' ? `R$ ${imovel.preco.toLocaleString('pt-BR')}/mês` : `R$ ${imovel.preco.toLocaleString('pt-BR')}`}</h5>
              </div>
          </div>
      </div>
  `).join('');
  ...
  return `
      <div class="container-fluid bg-sand p-5">
          <div class="container">
              <div class="row">
                  <div class="col-9">
                      <h2 class="home-features fw-normal">${titulo}</h2>
                  </div>
                  <div class="col-3 text-end">
                      <a href="#" class="btn btn-outline-light">Ver tudo</a>
                  </div>
              </div>
          </div>
      </div>
...
const secoes = [
  { titulo: "Lançamentos", ids: ['AP01', 'FA01', 'CH01'] },
  { titulo: "Mais Procurados", ids: ['SO01', 'CB01', 'AP02'] },
  { titulo: "Alto Padrão", ids: ['CO01', 'CA02', 'ES01'] },
  { titulo: "Ofertas Especiais", ids: ['CA01', 'SI01', 'FL01'] }
];

document.getElementById('destaques').innerHTML = secoes.map(secao => gerarDestaques(secao.titulo, secao.ids)).join('');
